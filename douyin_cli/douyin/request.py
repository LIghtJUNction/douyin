...
def _load_sign_script():
    """加载 JS 签名脚本"""
    js_file = Path(__file__).resolve().parent / "js" / "douyin.js"
...
class Request:
                ...
                self.HEADERS.update(
                    {
                        "User-Agent": UA,
                        "sec-ch-ua": (
                            f'"Chromium";v="{_version}", '
                            '"Not(A:Brand";v="24", '
                            f'"Google Chrome";v="{_version}"'
                        ),
                    },
                )
                self.PARAMS.update(
    ...
    def get_params(self, params: dict) -> dict:
        ...
        params["device_memory"] = self.COOKIES.get(
            CookieField.DEVICE_WEB_MEMORY_SIZE,
            8,
        )
    ...
    def getJSON(self, uri: str, params: dict, data: dict | None = None):
        url = f"{self.HOST}{uri}"
        ...
        if response.status_code != 200:
            logger.error(
                (
                    f"JSON请求失败：url: {url},  params: {params}, "
        ...
        if response.text == "":
            logger.error(
                (
                    f"响应体为空（Cookie可能已失效）：url: {url},  "
                    f"params: {params}, code: {response.status_code}"
                ),
            )
            raise CookieExpiredError(
                "Cookie可能已失效，响应体为空，请在设置中更新Cookie",
            )
        ...
        try:
            ...
            logger.error(
                (
                    f"JSON解析失败：url: {url},  params: {params}, "
                    f"body: {response.text}, error: {e}"
                ),
            )
            return {}
