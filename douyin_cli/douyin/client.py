...
from douyin_cli.text import abort
...
class DouyinClient:
    ...
    def _build_common_params(self, max_cursor: int, count: int | None = None) -> dict:
        """构建通用参数"""
        return {
            "cursor": max_cursor,
    ...
    def fetch_comments(
        ...
        resp = self.request.getJSON(APIEndpoint.COMMENT_LIST, params)
        return (
            resp.get("comments") or [],
            resp.get("cursor", 0),
            bool(resp.get("has_more", 0)),
        )
    ...
    def fetch_comment_replies(
        ...
        resp = self.request.getJSON(APIEndpoint.COMMENT_REPLY, params)
        return (
            resp.get("comments") or [],
            resp.get("cursor", 0),
            bool(resp.get("has_more", 0)),
