...
from douyin_cli.text import sanitize_filename, save_json
...
class DataParser:
    ...
    def _parse_single_aweme(item: dict, type: str) -> dict | None:
        ...
        if _type <= AwemeType.VIDEO_MAX or _type in AwemeType.VIDEO_SPECIAL:  # 视频
            ...
            if not images:
                logger.warning(
                    f"图文作品 {item.get('aweme_id', '未知')} 缺少 images 字段",
                )
                return None
            aweme["download_addr"] = [
                img.get("url_list", img.get("urlList"))[-1] for img in item["images"]
            ]
